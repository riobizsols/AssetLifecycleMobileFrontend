...
export default function DepartmentAssetAssignment() {
  ...
  const { assetId, barcode, departmentId, departmentName, assetData } = route.params || {};
  // Prioritize serial number from API response (assetData), then barcode, then empty string
  const [serial] = useState(assetData?.serial_number || assetData?.serialNumber || barcode || "");
  ...
  const fetchDepartments = async () => {
    ...
    try {
      ...
      // If departmentId was passed, set it
      if (departmentId && transformedDepartments.find(d => d.value === departmentId)) {
  ...
  const createAssetAssignment = async () => {
    ...
    try {
      ...
      const assignmentData = {
        ...
        asset_id: assetId,
        org_id: "ORG001",
        employee_int_id: null, // No employee for department-only assignment
        action: "A", // Assign action
        action_on: effectiveDate.toISOString(),
        action_by: "EMP001",
      ...
      if (!response.ok) {
        ...
        if (errorData.err && errorData.err.code === "23503") {
          if (errorData.err.constraint === "tbl_Departments_FK") {
